...
# bp()
# this is for sanity check the model quality
# for i in range(10):
#     print(f"{i}:", sep=" ")
#     print(tok.decode(model.generate(tok.encode("A few months later he returned home.", return_tensors="pt"), do_sample=True,temperature=0.7, max_length=100)[0]))
# input_raw = """
...
# assert inputs['input_ids'].shape[-1] > model.config.lsh_attn_chunk_length
...
# for input_raw in inputs_lst:
#     inputs = tok(input_raw, return_tensors="pt")
    
#     seq_len = inputs['input_ids'].shape[-1]
#     print(f"Seq_len({seq_len}) { '<' if seq_len <= chunk_len else '>'} ChunkLen({chunk_len})")
#     print(f"{'Using LSHAttn' if seq_len > chunk_len else 'FullAttn'}: ")
#     outputs = model(**inputs, labels=inputs["input_ids"])
#     loss = outputs.loss
#     logits = outputs.logits
...
for input_raw in long_inputs_lst:
    ...
    set_chunk_length(model, seq_len * 2)
    # bp()
