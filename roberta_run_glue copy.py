...
def convert_models():
    for masking_strategy in ['mp0.15', 'mp0.4', 'seq-len']:
        pytorch_dump_folder_path = f'/nas/home/nanx/Masking/repositories/fairseq/roberta_pytorch_models/{masking_strategy}'
        if os.path.exists(pytorch_dump_folder_path):
            shutil.rmtree(pytorch_dump_folder_path)
        convert_roberta_checkpoint_to_pytorch(
            roberta_checkpoint_path=f'/nas/home/nanx/Masking/repositories/fairseq/roberta_fairseq_models/{masking_strategy}',
...
def main(args):
    ...
    hyper_params = json.load(open(f'/nas/home/nanx/Masking/repositories/fairseq/examples/roberta/config/finetuning_hf/{args.task}.json', 'r'))
    hyper_params['model_name_or_path'] = f'/nas/home/nanx/Masking/repositories/fairseq/roberta_fairseq_models/{args.masking}'
    """
    convert models
    """
    if not os.path.exists(os.path.join(hyper_params['model_name_or_path'], 'pytorch_model.bin')):
        model_name = sorted([i for i in os.listdir(hyper_params['model_name_or_path']) if i.startswith('checkpoint')])[::-1][0]
        logger.info(f'converting fairseq model {model_name} to pytorch model...')
        ...
        convert_roberta_checkpoint_to_pytorch(
            roberta_checkpoint_path=hyper_params['model_name_or_path'],
            pytorch_dump_folder_path=hyper_params['model_name_or_path'],
            classification_head=False
        )
    hyper_params['project_name'] = f'roberta_{args.masking}_{args.task}'
    hyper_params['output_dir'] = f'/nas/home/nanx/Masking/repositories/fairseq/results_glue/roberta/{args.masking}/{args.task}/lr{hyper_params["learning_rate"]}_B{hyper_params["per_device_train_batch_size"]}_E{hyper_params["num_train_epochs"]}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-masking', type=str, required=True)
